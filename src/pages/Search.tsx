...
export default function SearchPage() {
  ...
  // Tabs
  const [activeTab, setActiveTab] = useState<'Provider' | 'Post' | 'Service'>(
    'Post',
  );
    ...
    const fetchInitialData = async () => {
      ...
      if (activeTab === 'Provider') {
        ...
        if (providers.length === 0) {
          // fallback: show all providers or first few
  ...
  return (
    <>
      ...
      <main className="flex flex-col items-center min-h-screen px-6 py-8 gap-6 bg-gray-50">
        ...
        {/* SERVICE TAB */}
        {activeTab === 'Service' && (
          <div className="w-full max-w-6xl text-center text-gray-700 text-lg mt-8">
            🧰 Service catalog will be shown here.
          </div>
        )}
      </main>
      <Footer />
