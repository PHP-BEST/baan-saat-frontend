...
import { createApply, type ApplyFormInterface } from '@/api/apply';
...
export default function ApplyCreatePage() {
  ...
  useEffect(() => {
    const getCurrentPost = async () => {
      ...
      setPost(post);
      if (post) {
        setFormData((prev) => ({ ...prev, budget: post.budget }));
        const currentCustomer = await getUserById(post.customerId);
  ...
  const handleSubmit = async (e?: FormEvent<HTMLFormElement>) => {
    ...
    if (canSubmit) {
      setAdding(true);
      const success = await createApply(
        formData,
        post.customerId,
        user._id,
        post._id,
      ...
      <main className="flex-grow flex justify-center py-12 px-4">
        <div className="w-[65%] min-h-full bg-gray-100 rounded-2xl border border-gray-300 p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Post Cover Image */}
            {(post.coverPhotoUrl ?? post.image1Url) ? (
              <img
                src={(post.coverPhotoUrl ?? post.image1Url) as string}
                alt={post.title}
